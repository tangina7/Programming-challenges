...
def ExecuteLDAimm(Registers, Operand):
    """Decription: Copies the value from immediate addressing in the accumulator. 
       Return: Array"""
...
def ExecuteADD(Memory, Registers, Address):
    """Decription: Adds the value from the memory address to the value in accumulator. Checks for overflow.
       Return: Array"""
...
def ExecuteSUB(Memory, Registers, Address):
    """Decription: Subtracts the value from the memory address to he value in the accumulator. Checks for overflow.
       Return: Array"""
    Registers[ACC] = Registers[ACC] - Memory[Address].OperandValue
    Registers = SetFlags(Registers[ACC], Registers)
    if Registers[STATUS] == ConvertToDecimal("001"):
...
def ExecuteCMPimm(Registers, Operand):
    """Decription: Compares the accumulator value with operand using immediate addressing using subtraction. The result is used to set the flags for the calculation.
       Return: Array"""
...
def ExecuteBEQ(Registers, Address):
    """Decription: The base ten integer in the status register is converted to binary, If the first index, which is the z flag is "1", the next instruction changes to the memory loacttion specified. 
       Return: Array"""
...
def ExecuteJMP(Registers, Address):
    """Decription: Jumps to the memory location specified by changing the program counter.
       Return: Array"""
    Registers[PC] = Address
    return Registers
